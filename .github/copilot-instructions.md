...
- Shared pairing/device logic is in `drivers/BaseDriver.js` and `drivers/BaseDevice.js`.
- Driver-specific manifests and settings are in each driver folder under `drivers/`.

# Homey Cloud Rules

- **Applies to cloud drivers**: These rules apply to drivers that are intended to run on Homey Cloud (platforms include cloud).
