...
module.exports = class Sensibo {
  ...
  async setAcState(acState) {
    ...
    if (response.status !== 200) {
      if (this.isPodNotConnectedResponse(response)) {
        this._logger('setAcState skipped because pod is not connected', this.getDeviceId(), acStatePatch, this.getErrorMessageFromResponse(response));
        return response.data;
      }
      throw new Error(`Error setting AC state (${this.getErrorMessageFromResponse(response)})`);
    }
    return response.data;
  }
